...
fn count_cached_statuses(outcomes: &[RslipOutcome]) -> (usize, usize, usize) {
    ...
    for outcome in outcomes {
        match outcome.status {
            rpytest_runner::TestStatus::Passed => passed += 1,
            rpytest_runner::TestStatus::Failed => failed += 1,
            rpytest_runner::TestStatus::TimedOut => timed_out += 1,
        }
    }
...
fn cached_status_label(status: rpytest_runner::TestStatus) -> &'static str {
    match status {
...
mod tests {
    use super::*;
    use crate::{CacheStatus, LineCoverage};
