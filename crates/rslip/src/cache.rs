...
impl From<&RslipOutcome> for CacheEntry {
...
pub(crate) fn load_cache_entry(cache_root: &Path, fingerprint: &str) -> Option<CacheEntry> {
    let path = cache_path(cache_root, fingerprint);
...
pub(crate) fn store_cache_entry(
    ...
    let tmp_path = parent.join(format!(".{}.{}.tmp", fingerprint, unique_suffix()));
...
pub(crate) fn cache_path(cache_root: &Path, fingerprint: &str) -> PathBuf {
    cache_root
        .join("entries")
...
pub(crate) fn cache_fingerprint(req: &RslipRequest) -> io::Result<String> {
    ...
    for file in conservative_input_files(&req.cwd)? {
        h = fnv1a64(h, file.to_string_lossy().as_bytes());
        h = fnv1a64(h, &[0]);
        h = fnv1a64(h, &fs::read(file)?);
        h = fnv1a64(h, &[0]);
    }
    Ok(format!("{h:016x}"))
}

pub(crate) fn conservative_input_files(root: &Path) -> io::Result<Vec<PathBuf>> {
    let mut out = Vec::new();
    visit_inputs(root, &mut out)?;
    out.sort_by(|a, b| a.to_string_lossy().cmp(&b.to_string_lossy()));
    Ok(out)
}

fn visit_inputs(dir: &Path, out: &mut Vec<PathBuf>) -> io::Result<()> {
    for entry in fs::read_dir(dir)? {
        ...
        if file_type.is_dir() {
            ...
            visit_inputs(&path, out)?;
        } else if file_type.is_file() && is_cache_input(&path) {
            out.push(path);
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn cache_fingerprint_changes_when_python_content_changes() {
        let tmp = tempfile::tempdir().unwrap();
        fs::write(tmp.path().join("pkg.py"), "def value():\n    return 1\n").unwrap();
        let req = crate::sample_request(tmp.path());
    ...
    fn conservative_inputs_include_pytest_config_and_skip_cache_dirs() {
        ...
        let names: BTreeSet<_> = conservative_input_files(tmp.path())
            .unwrap()
            .into_iter()
