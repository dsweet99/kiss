...
#[cfg(any(debug_assertions, test))]
pub(crate) const BARRIER_DIR_ENV: &str = "KISS_QA_PUBLICATION_BARRIER_DIR";
#[cfg(any(debug_assertions, test))]
pub(crate) const BARRIER_TARGET_ENV: &str = "KISS_QA_PUBLICATION_BARRIER_TARGET";
#[cfg(debug_assertions)]
...
#[cfg(debug_assertions)]
pub(crate) fn read_release_record(path: &Path) -> io::Result<Option<ReleaseRecord>> {
...
#[cfg(debug_assertions)]
pub(crate) fn validate_release_record(
...
#[cfg(debug_assertions)]
pub(crate) fn ensure_child_path(dir: &Path, path: &Path) -> io::Result<()> {
...
#[cfg(debug_assertions)]
pub(crate) fn operation_id(artifact: &str, phase: &str, temporary_path: &Path) -> String {
...
#[cfg(debug_assertions)]
pub(crate) fn json_number_field(text: &str, field: &str) -> io::Result<u32> {
...
#[cfg(debug_assertions)]
pub(crate) fn json_string_field(text: &str, field: &str) -> io::Result<String> {
    let marker = format!("\"{field}\"");
    let start = text
        .find(&marker)
        .ok_or_else(|| io::Error::other(format!("missing JSON field {field}")))?;
