...
use crate::batch_entry_state::read_entry_state;
...
fn test_name_for_mode(mode: &str) -> &'static str {
    match mode {
        "publish" => {
            "batch_reverse_line_index::process_race_tests::two_os_process_publishers_single_manifest_activation"
        }
        "publish_barrier" | "reader" => {
            "batch_reverse_line_index::process_race_tests::os_process_readers_during_pre_manifest_pause_never_see_partial"
        }
...
fn child_context() -> (String, PathBuf, crate::RustCoverageBatchRequest) {
