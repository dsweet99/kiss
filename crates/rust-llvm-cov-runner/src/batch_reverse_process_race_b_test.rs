...
use crate::batch_reverse_process_race_support::{
    assert_ok, child_work_and_repo, release_barrier, spawn_exact, wait_barrier_ready, wait_path,
    wait_ready, SpawnExact,
};
use crate::batch_reverse_publish::{prune_unreferenced_snapshots, snapshot_path};
use crate::batch_reverse_test_support::seed_alpha_beta_reverse;
use crate::publish_derived_state;
use crate::query_reverse_line_index;
...
fn primed() -> (tempfile::TempDir, crate::RustCoverageBatchRequest, PathBuf) {
...
fn repair_under_lock(req: &crate::RustCoverageBatchRequest) {
    let tools = witness_batch_tools();
...
fn assert_reader_safe(req: &crate::RustCoverageBatchRequest, generation: &str, prior: &str) {
...
fn publish_under_lock(req: &crate::RustCoverageBatchRequest) -> String {
    let tools = witness_batch_tools();
    let identity = crate::batch_fingerprint::batch_identity(req, &tools).unwrap();
    let _guard = lock_batch(&req.cache_root).unwrap();
    publish_derived_state(
...
fn child_ctx() -> (String, PathBuf, crate::RustCoverageBatchRequest) {
...
fn test_name(mode: &str) -> &'static str {
    match mode {
        "publish_hold_after_manifest" | "publish_after_go" | "reader" => {
            "batch_reverse_line_index::process_race_b_tests::os_process_post_manifest_publisher_b_serializes_on_batch_lock"
        }
        "publish_barrier_pre_manifest" => {
            "batch_reverse_line_index::process_race_b_tests::os_process_kill_at_entry_state_barrier_then_repair"
        }
        "prune_hold" => {
            "batch_reverse_line_index::process_race_b_tests::os_process_prune_never_deletes_manifest_active_snapshot"
        }
