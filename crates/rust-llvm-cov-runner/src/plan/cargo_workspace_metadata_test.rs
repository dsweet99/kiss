...
fn workspace_package_record_fields_are_accessible_to_tests() {
    let record = crate::plan::cargo_workspace_metadata::WorkspacePackageRecord {
...
fn effective_manifest_path_supports_split_and_joined_flags() {
    let joined = crate::plan::cargo_workspace_metadata::effective_manifest_path(
        std::path::Path::new("/repo"),
        &["--manifest-path=/joined/Cargo.toml".to_string()],
    );
    let split = crate::plan::cargo_workspace_metadata::effective_manifest_path(
        std::path::Path::new("/repo"),
        &[
...
fn cargo_metadata_accessors_expose_workspace_fields() {
    let metadata = crate::plan::cargo_workspace_metadata::cargo_metadata_witness_for_test();
    ...
    let package = crate::plan::cargo_workspace_metadata::workspace_package_for_test(
        "pkg-id",
        "pkg",
        PathBuf::from("/repo"),
    );
...
fn current_package_id_matches_workspace_manifest() {
    ...
    std::fs::create_dir_all(tmp.path().join("src")).unwrap();
    std::fs::write(tmp.path().join("src").join("lib.rs"), "pub fn x() {}\n").unwrap();
    let metadata = crate::plan::cargo_workspace_metadata::load_cargo_metadata(
        tmp.path(),
        std::path::Path::new("cargo"),
        &[],
    )
...
fn effective_manifest_path_honors_manifest_path_flag() {
    let path = crate::plan::cargo_workspace_metadata::effective_manifest_path(
        std::path::Path::new("/repo"),
