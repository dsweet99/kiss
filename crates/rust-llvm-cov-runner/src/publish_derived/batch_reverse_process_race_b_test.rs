...
fn test_name(mode: &str) -> &'static str {
    match mode {
        "publish_hold_after_manifest" | "publish_after_go" | "reader" => {
            "publish_derived::batch_reverse_line_index::process_race_b_tests::os_process_post_manifest_publisher_b_serializes_on_batch_lock"
        }
        "publish_barrier_pre_manifest" => {
            "publish_derived::batch_reverse_line_index::process_race_b_tests::os_process_kill_at_entry_state_barrier_then_repair"
        }
        "prune_hold" => {
            "publish_derived::batch_reverse_line_index::process_race_b_tests::os_process_prune_never_deletes_manifest_active_snapshot"
        }
