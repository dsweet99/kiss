...
fn read_population_and_index_loaders_reject_invalid_json() {
    ...
    let index = crate::publish_derived::batch_derived_index_types::OnDiskIndex {
        schema_version: INDEX_SCHEMA_VERSION.to_string(),
        generation_fingerprint: "gen".to_string(),
        entries_fingerprint: "entries".to_string(),
    };
    let manifest = crate::publish_derived::batch_derived_index_types::PopulationManifestOnDisk {
...
fn alpha_entry(generation: &str, files: BTreeMap<String, BTreeSet<u32>>) -> RustCovCacheEntry {
...
fn publish_under_batch_lock(
    ...
    let _guard = crate::execute_or_reuse::batch_lock::lock_batch(&req.cache_root).unwrap();
...
fn concurrent_repairers_observe_single_repair() {
    ...
    assert!(
        population_manifest_state_is_current(
            &req.cache_root,
            &req.source_root,
            &identity,
            &selectors
...
fn publish_derived_state_fails_when_retention_pruning_cannot_remove_stale_entry() {
    ...
    let err =
        publish_derived_state(&req, &tools, &identity, &["alpha".to_string()], false).unwrap_err();
    let mut permissions = std::fs::metadata(&entries_dir).unwrap().permissions();
...
fn publish_derived_state_fails_when_population_manifest_parent_is_not_writable() {
    ...
    let err =
        publish_derived_state(&req, &tools, &identity, &["alpha".to_string()], true).unwrap_err();
    assert!(matches!(err, RustLlvmCovError::Io(_)));
}
