...
fn verify_python_defaults(c: &Config) {
    ...
    assert_eq!(c.arguments_positional, 3);
    assert_eq!(c.arguments_keyword_only, 3);
    assert_eq!(c.max_indentation_depth, 4);
    assert_eq!(c.nested_function_depth, 2);
    assert_eq!(c.returns_per_function, 5);
    assert_eq!(c.return_values_per_function, 3);
    assert_eq!(c.branches_per_function, 9);
    assert_eq!(c.local_variables_per_function, 15);
    assert_eq!(c.statements_per_try_block, 3);
    assert_eq!(c.boolean_parameters, 1);
    assert_eq!(c.annotations_per_function, 5);
    assert_eq!(c.calls_per_function, 20);
    assert_eq!(c.statements_per_file, 200);
    assert_eq!(c.lines_per_file, 300);
    assert_eq!(c.functions_per_file, 10);
    assert_eq!(c.interface_types_per_file, 1);
    assert_eq!(c.concrete_types_per_file, 1);
    assert_eq!(c.imported_names_per_file, 30);
    assert_eq!(c.cycle_size, 0);
    assert_eq!(c.indirect_dependencies, 10);
    assert_eq!(c.dependency_depth, 3);
}
...
fn verify_rust_defaults(c: &Config) {
    ...
    assert_eq!(c.returns_per_function, 5);
    assert_eq!(c.branches_per_function, 9);
    assert_eq!(c.local_variables_per_function, 20);
    assert_eq!(c.boolean_parameters, 1);
    assert_eq!(c.annotations_per_function, 1);
    assert_eq!(c.calls_per_function, 45);
    assert_eq!(c.methods_per_class, 10);
    assert_eq!(c.statements_per_file, 250);
    assert_eq!(c.lines_per_file, 900);
    assert_eq!(c.functions_per_file, 40);
    assert_eq!(c.interface_types_per_file, 0);
    assert_eq!(c.concrete_types_per_file, 8);
    assert_eq!(c.imported_names_per_file, 50);
    assert_eq!(c.cycle_size, 0);
    assert_eq!(c.indirect_dependencies, 10);
    assert_eq!(c.dependency_depth, 3);
}
...
fn default_config_toml_matches_requested_defaults() {
    ...
    verify_python_defaults(&py_config);
    verify_rust_defaults(&rs_config);
}
