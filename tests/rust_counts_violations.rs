...
fn test_statements_per_function_violation() {
    ...
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { statements_per_function: 10, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "statements_per_function");
    assert!(has_violation, "should trigger statements_per_function violation");
}
...
fn test_arguments_per_function_violation() {
    ...
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { arguments_per_function: 3, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "arguments_per_function");
    assert!(has_violation, "should trigger arguments_per_function violation");
}
...
").unwrap();
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { max_indentation_depth: 2, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "max_indentation_depth");
    assert!(has_violation, "should trigger max_indentation_depth violation");
}
...
").unwrap();
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { returns_per_function: 2, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "returns_per_function");
    assert!(has_violation, "should trigger returns_per_function violation");
}
...
").unwrap();
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { branches_per_function: 3, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "branches_per_function");
    assert!(has_violation, "should trigger branches_per_function violation");
}
...
fn test_local_variables_per_function_violation() {
    ...
    write!(tmp, "{code}").unwrap();
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { local_variables_per_function: 5, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "local_variables_per_function");
    assert!(has_violation, "should trigger local_variables_per_function violation");
}
...
").unwrap();
    let parsed = parse_rust_file(tmp.path()).unwrap();
    let config = Config { nested_function_depth: 1, ..Default::default() };
    let violations = analyze_rust_file(&parsed, &config);
    let has_violation = violations.iter().any(|v| v.metric == "nested_closure_depth");
    assert!(has_violation, "should trigger nested_closure_depth violation");
}
