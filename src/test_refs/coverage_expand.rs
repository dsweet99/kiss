...
pub(crate) fn expand_py_refs_via_production_imports(
    ...
    for parsed in parsed_files {
        ...
        // OI concrete modules: facade imports (evaluate.py, objectdb.py, …) must not
        // propagate witness to sibling defs llvm never executes (rope objectdb inflation).
        if is_py_base_oi_subtree(&parsed.path) && !is_py_oi_interfaces_stub_path(&parsed.path) {
            continue;
        }
        let file_defs: Vec<&CodeDefinition> = definitions
            .iter()
