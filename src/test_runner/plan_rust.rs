...
fn rust_all_mode_population_required(repo_root: &Path, planned: &[String]) -> bool {
    ...
    if kiss::rust_llvm_cov_runner::current_population_manifest_matches_source_universe(
        cache_root.as_path(),
        identity,
        planned,
    ) == Some(true)
    ...
    {
        !current
    } else if let Some(state) = kiss::rust_llvm_cov_runner::load_current_population_state(
        cache_root.as_path(),
        repo_root,
        identity,
        None,
    ) {
        state.selectors != *planned
    } else {
