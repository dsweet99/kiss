...
/// Scripted source for nudge-loop tests (distinct from settle-script in session_test).
struct NudgeScript {
...
fn py_dry_args() -> RunTestCmdArgs<'static> {
    RunTestCmdArgs {
...
fn commit_a_py(tmp: &tempfile::TempDir) -> PathBuf {
...
fn timeout_steps(n: usize) -> VecDeque<Result<Vec<NormalizedWatchEvent>, RecvTimeout>> {
