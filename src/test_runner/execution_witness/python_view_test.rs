...
use crate::test_runner::execution_witness::accept::{WitnessScope, WitnessStatus};
...
use super::python_view::{python_identity_digest, python_witness_from_pinned};
...
#[test]
fn incomplete_generation_is_subset_scope() {
    let pinned = sample_pinned(false);
    let witness = python_witness_from_pinned(&pinned);
    assert_eq!(witness.scope, WitnessScope::Subset);
}
