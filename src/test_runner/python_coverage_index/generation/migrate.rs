...
use super::super::coverage_snapshot::try_load_python_coverage_snapshot;
use super::super::manifest::{
    PYTHON_COVERAGE_ENV_KEYS, read_python_population_manifest,
    stored_python_universe_population,
};
use super::super::storage::{load_current_python_coverage_index, python_coverage_cache_root};
...
fn load_complete_v1_bundle(
    ...
    if manifest.schema_version != super::super::POPULATION_SCHEMA_VERSION
...
fn load_v1_durations_for_migrate(
    repo_root: &Path,
    test_args: &[String],
    population: &Option<super::super::manifest::StoredPythonPopulation>,
) -> Option<Vec<(String, Duration)>> {
    let population = population.as_ref()?;
    let manifest = read_python_population_manifest(repo_root)?;
    let cache_root = python_coverage_cache_root(repo_root).ok()?;
    if let Some(cached) =
        super::super::population_durations::try_load_population_durations(&cache_root, &manifest)
    {
        return Some(cached);
    }
    super::super::population_durations::load_durations_from_entry_probes_allow_non_passed(
        repo_root,
        test_args,
