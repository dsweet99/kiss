...
pub(super) fn merge_test(merged: &mut toml::Table, ex: &toml::Table, nw: &toml::Table) {
    // Keep an existing [test] (user may have customized jobs/plugins/ignore).
    // Otherwise take the newly generated defaults so mimic never drops the section.
    if let Some(v) = ex.get("test").cloned().or_else(|| nw.get("test").cloned()) {
        merged.insert("test".to_string(), v);
    }
}
...
pub fn build_merged_output(m: &toml::Table) -> String {
    ...
    for k in ["gate", "python", "rust", "test", "shared", "thresholds"] {
