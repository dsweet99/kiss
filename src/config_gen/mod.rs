...
pub use generate::{generate_config_toml_by_language, GenerateConfigParams};
pub use infer_gate::infer_gate_config_for_paths;
pub use merge::{merge_config_toml, MergeLanguageUpdate};
...
pub fn write_mimic_config(
    ...
    let content = if out.exists() {
        merge::merge_config_toml(out, toml, merge::MergeLanguageUpdate::from_analyzed_counts(
            py_cnt, rs_cnt,
        ))
    } else {
        toml.to_string()
