...
pub(super) const RS_RULE_SPECS: &[RuleSpec] = &[
    ...
    RuleSpec {
        ...
        description: "test_coverage_threshold is the minimum percent of syntactically coverable source lines covered by cached llvm-cov runtime coverage. Enforcement is per file or codebase-wide according to `test_coverage_scope` (default `codebase`). `kiss test` uses a current cache; when Rust coverage is stale, it reuses selector coverage only when the compiled test executable digest is unchanged, reruns invalidated selectors, and falls back to a full Rust test-population refresh whenever reuse cannot be proven safe.",
    },
    RuleSpec {
