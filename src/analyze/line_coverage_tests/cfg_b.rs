use super::*;
    ...
    fn cfg_attrs_and_expr_error_paths_are_conservative() {
        ...
        let name_value_cfg: syn::Item =
            syn::parse_str("#[cfg = \"unix\"] struct Named;").unwrap();
    ...
    fn cfg_helpers_fuzz_random_expr_kinds_stay_boolean() {
        ...
        let mut state = seed;
        let corpus = [
            "1", "x", "f()", "x.y", "xs[0]", "&x", "*x", "-x", "1+2", "{1}", "(1,2)", "async {1}",
            "match x { _ => 1 }", "if true {1} else {0}", "loop { break }", "while false {}",
        ];
        for _ in 0..64 {
    ...
    fn coverage_denominator_visits_impl_methods_and_cfg_blocks() {
        ...
        assert!(denom.len() >= 3, "expected impl/block statements in denom, got {denom:?}");
    }
    ...
    fn coverage_denominator_returns_empty_for_unreadable_path() {
        let missing = std::path::PathBuf::from("/tmp/kiss-missing-coverage-denom-file-does-not-exist.rs");
