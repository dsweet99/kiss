...
use super::line_coverage_cfg::{cfg_expr_active, literal_string_value};
    ...
    fn cfg_helpers_cover_item_and_expr_variants_exhaustively() {
        use super::line_coverage_cfg::{
            expr_cfg_active, item_cfg_active, stmt_cfg_active,
        };
        ...
        for snippet in item_snippets {
            ...
            assert!(stmt_cfg_active(&stmt), "stmt item should be active: {snippet}");
        }
        ...
        for snippet in expr_snippets {
            ...
            assert!(stmt_cfg_active(&stmt), "stmt expr should be active: {snippet}");
        }
        assert!(parsed >= 30, "expected most expr snippets to parse, got {parsed}");
