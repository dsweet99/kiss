...
pub fn generate_config_toml_by_language(py: &MetricStats, rs: &MetricStats, py_n: usize, rs_n: usize) -> String {
    ...
    let _ = writeln!(out, "test_coverage_threshold = {}", defaults::gate::TEST_COVERAGE_THRESHOLD);
    let _ = writeln!(out, "min_similarity = {}\n", defaults::duplication::MIN_SIMILARITY);
    if py_n > 0 { 
        append_section(&mut out, "[python]", &compute_summaries(py), python_config_key);
        append_python_non_collected_defaults(&mut out);
    } else {
        append_python_defaults(&mut out);
    }
    if rs_n > 0 { 
        append_section(&mut out, "[rust]", &compute_summaries(rs), rust_config_key);
        append_rust_non_collected_defaults(&mut out);
    } else {
...
fn append_python_non_collected_defaults(out: &mut String) {
    use crate::defaults::python;
    use std::fmt::Write;
    let _ = writeln!(out, "statements_per_try_block = {}", python::STATEMENTS_PER_TRY_BLOCK);
    let _ = writeln!(out, "boolean_parameters = {}", python::BOOLEAN_PARAMETERS);
    let _ = writeln!(out, "decorators_per_function = {}", python::DECORATORS_PER_FUNCTION);
    let _ = writeln!(out, "cycle_size = {}\n", crate::defaults::graph::CYCLE_SIZE);
}

fn append_rust_non_collected_defaults(out: &mut String) {
    use crate::defaults::rust;
    use std::fmt::Write;
    let _ = writeln!(out, "boolean_parameters = {}", rust::BOOLEAN_PARAMETERS);
    let _ = writeln!(out, "attributes_per_function = {}", rust::ATTRIBUTES_PER_FUNCTION);
    let _ = writeln!(out, "cycle_size = {}\n", crate::defaults::graph::CYCLE_SIZE);
...
mod tests {
    ...
    fn test_defaults_appenders() {
        let mut py_nc = String::new();
        append_python_non_collected_defaults(&mut py_nc);
        assert!(py_nc.contains("statements_per_try_block") && py_nc.contains("boolean_parameters"));
        let mut rs_nc = String::new();
        append_rust_non_collected_defaults(&mut rs_nc);
        assert!(rs_nc.contains("boolean_parameters") && rs_nc.contains("attributes_per_function"));
