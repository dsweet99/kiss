...
fn compute_min_per_file_test_coverage(py_parsed: &[ParsedFile], rs_parsed: &[ParsedRustFile]) -> usize {
    let mut definitions: Vec<(PathBuf, String, usize)> = Vec::new();
    let mut unreferenced: Vec<(PathBuf, String, usize)> = Vec::new();
...
fn has_orphan_modules(py_parsed: &[ParsedFile], rs_parsed: &[ParsedRustFile]) -> bool {
