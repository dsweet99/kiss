...
use crate::duplication::{cluster_duplicates, detect_duplicates_from_chunks, extract_chunks_for_duplication, extract_rust_chunks_for_duplication, DuplicationConfig};
...
pub fn generate_config_toml_by_language(py: &MetricStats, rs: &MetricStats, py_n: usize, rs_n: usize, gate: &GateConfig) -> String {
    use std::fmt::Write;
    let mut out = String::from("# Generated by kiss mimic\n# Thresholds based on max values of analyzed codebase\n\n");
    let _ = writeln!(out, "[gate]");
    let _ = writeln!(out, "test_coverage_threshold = {}", gate.test_coverage_threshold);
    let _ = writeln!(out, "min_similarity = {}", gate.min_similarity);
    let _ = writeln!(out, "duplication_enabled = {}\n", gate.duplication_enabled);
    if py_n > 0 { 
        append_section(&mut out, "[python]", &compute_summaries(py), python_config_key);
    } else {
...
pub fn infer_gate_config_for_paths(paths: &[String], lang: Option<Language>, ignore: &[String]) -> GateConfig {
    ...
    let py_parsed = if py_files.is_empty() {
        ...
        parse_files(&py_files).ok().into_iter().flatten().filter_map(Result::ok).collect::<Vec<ParsedFile>>()
    };
    let rs_parsed = if rs_files.is_empty() {
        Vec::new()
    } else {
        parse_rust_files(&rs_files).into_iter().filter_map(Result::ok).collect::<Vec<ParsedRustFile>>()
    };

    gate.test_coverage_threshold = compute_static_test_coverage(&py_parsed, &rs_parsed);
    gate.duplication_enabled = !has_reportable_duplicates(&py_parsed, &rs_parsed, gate.min_similarity);
    gate
}

fn gather_files_by_lang(paths: &[String], lang: Option<Language>, ignore: &[String]) -> (Vec<PathBuf>, Vec<PathBuf>) {
...
fn has_reportable_duplicates(py_parsed: &[ParsedFile], rs_parsed: &[ParsedRustFile], min_similarity: f64) -> bool {
...
fn append_python_defaults(out: &mut String) {
    ...
    let _ = writeln!(out, "statements_per_function = {}", python::STATEMENTS_PER_FUNCTION);
    ...
    let _ = writeln!(out, "branches_per_function = {}", python::BRANCHES_PER_FUNCTION);
...
fn append_rust_defaults(out: &mut String) {
    ...
    let _ = writeln!(out, "statements_per_function = {}", rust::STATEMENTS_PER_FUNCTION);
    let _ = writeln!(out, "arguments = {}", rust::ARGUMENTS);
    let _ = writeln!(out, "max_indentation = {}", rust::MAX_INDENTATION);
    let _ = writeln!(out, "branches_per_function = {}", rust::BRANCHES_PER_FUNCTION);
...
mod tests {
    ...
    fn test_config_generation() {
        ...
        assert!(generate_config_toml_by_language(&MetricStats::default(), &MetricStats::default(), 0, 0, &gate).contains("Generated by kiss"));
