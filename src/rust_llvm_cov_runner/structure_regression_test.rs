...
fn crate_src_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/rust_llvm_cov_runner")
}
...
fn file_has_forbidden_import(path: &std::path::Path, needles: &[&str]) -> Option<String> {
    ...
    for needle in needles {
        let crate_needle = format!("crate::rust_llvm_cov_runner::{needle}");
        let references_forbidden_module = text.contains(&format!("{crate_needle}::"))
            || text.contains(&format!("use {crate_needle}"))
            || (text.contains(&format!("{needle}::"))
                && text.lines().any(|line| {
                    line.contains(needle)
                        && (line.contains("use ") || line.contains("crate::rust_llvm_cov_runner::"))
                })
                && text.contains(&crate_needle));
        if references_forbidden_module {
            return Some(format!("{path:?} contains {crate_needle}"));
        }
    }
    None
}
