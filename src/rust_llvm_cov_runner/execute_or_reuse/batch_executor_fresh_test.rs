...
fn subprocess_exporter_wrapper_propagates_pre_export_failures() {
    ...
    let identity = batch_identity(&req, &tools).unwrap();
    let plan = build_rust_coverage_batch_plan(&req).unwrap();
    let exporter = crate::rust_llvm_cov_runner::execute_or_reuse::batch_export::SubprocessInstanceExporter::new(
        crate::rust_llvm_cov_runner::execute_or_reuse::batch_export_tools::ExportTools {
