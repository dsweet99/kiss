...
fn reject_missing_terminal_events_and_instance_profile_path_are_exercised() {
    ...
    assert!(
        matches!(err, crate::rust_llvm_cov_runner::RustLlvmCovError::InvalidRequest(message) if message.contains("missing terminal events"))
    );
...
fn finish_fresh_check_aggregate_store_failure_prevents_final_publication() {
    ...
    assert!(matches!(
        result.batch_error,
        Some(crate::rust_llvm_cov_runner::RustLlvmCovError::Io(_))
    ));
    assert_eq!(
