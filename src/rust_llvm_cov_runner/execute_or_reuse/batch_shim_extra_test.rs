...
fn target_runner_shim_routes_child_output_through_side_channel_only() {
    ...
    unsafe {
        std::env::remove_var(
            crate::rust_llvm_cov_runner::execute_or_reuse::batch_output_channel::OUTPUT_CHANNEL_SOCKET_ENV,
        );
        std::env::remove_var(
            crate::rust_llvm_cov_runner::execute_or_reuse::batch_output_channel::OUTPUT_CHANNEL_TOKEN_ENV,
        );
    }
    ...
    assert!(frames.iter().any(|frame| frame.bytes == b"child-out"
        && frame.stream
            == crate::rust_llvm_cov_runner::execute_or_reuse::batch_output_channel::OutputStreamKind::Stdout));
    assert!(frames.iter().any(|frame| frame.bytes == b"child-err"
        && frame.stream
            == crate::rust_llvm_cov_runner::execute_or_reuse::batch_output_channel::OutputStreamKind::Stderr));
}
