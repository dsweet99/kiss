...
fn resolve_included_packages_rejects_unknown_package_filter() {
    ...
    assert!(matches!(
        err,
        crate::rust_llvm_cov_runner::RustLlvmCovError::InvalidRequest(message) if message.contains("missing-package")
    ));
}

#[test]
fn resolve_ignore_filename_regex_fails_for_missing_manifest() {
    let req = crate::rust_llvm_cov_runner::plan::batch_plan::RustCoverageBatchRequest {
...
fn resolve_ignore_filename_regex_for_kiss_repo_root() {
    ...
    assert!(
        regex.contains("/tests/") || regex.contains("tests\\.rs"),
        "single-crate kiss ignore regex should still exclude test sources: {regex}"
    );
}
