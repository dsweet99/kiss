...
pub(crate) struct PopulationManifestOnDisk {
    ...
    pub(crate) test_binaries: BTreeMap<String, crate::rust_llvm_cov_runner::RustTestBinaryIdentity>,
    pub(crate) reverse_line_index: Option<ReverseLineIndexManifestMeta>,
}
...
) -> Option<BTreeMap<String, crate::rust_llvm_cov_runner::RustTestBinaryIdentity>> {
    let mut out = BTreeMap::new();
    let mut previous: Option<String> = None;
    for record in records {
        ...
        let item = crate::rust_llvm_cov_runner::RustTestBinaryIdentity {
            id: record.id.clone(),
