...
#[test]
fn test_foil058_cliff00_weighted_nonzero() {
    ...
    let parsed: Vec<_> = paths
        .iter()
        .filter_map(|p| crate::rust_parsing::parse_rust_file(p).ok())
        .collect();
    let refs: Vec<_> = parsed.iter().collect();
    let analysis = super::analyze_rust_test_refs(&refs, None);
    let cliff = root.join("src/cliffs/cliff_00.rs");
    let weighted = super::compute_rs_weighted_file_pcts(&analysis, &refs);
    let pct = weighted.get(&cliff).copied().unwrap_or(100);
    assert!(
        (1..=5).contains(&pct),
        "expected rust cliff weighted pct ~2%, got {pct}%"
    );
}
#[test]
fn test_foil058_orchestrate_in_coverage_map() {
    let root = std::path::Path::new("/tmp/kiss_foil_058jr492");
    if !root.exists() {
        return;
    }
    ...
    let analysis = super::analyze_rust_test_refs(&refs, None);
    let cliff = root.join("src/cliffs/cliff_00.rs");
    let key = (cliff.clone(), "orchestrate".to_string());
    assert!(
        analysis.coverage_map.contains_key(&key),
        "orchestrate should be in coverage_map, keys={:?}",
        analysis
            .coverage_map
            .keys()
            .filter(|(_f, n)| *n == "orchestrate")
            .collect::<Vec<_>>()
    );
}
