...
fn rs_branch_credit(
    ...
    if metrics.branches <= b_ref {
        ...
        let observed_paths = b_ref.saturating_add(1);
        let possible_paths = metrics.branches.saturating_add(1);
        observed_paths as f64 / possible_paths as f64
    }
}
...
fn rs_weighted_definition_credit(
    ...
    let covering = analysis
        ...
        .map(Vec::as_slice)
        .unwrap_or(&[]);
    let witnessed = analysis.call_references.contains(&def.name)
        || analysis.propagated_references.contains(&def.name);
    if covering.is_empty() && !witnessed {
        return 0.0;
    }
        ...
        && !analysis.propagated_references.contains(&def.name)
    {
        return rs_module_import_surface_credit(analysis, def, metrics, covering, parsed_by_path)
            .unwrap_or_else(|| rs_branch_credit(metrics, witnessed, covering, parsed_by_path));
    }
    rs_branch_credit(metrics, witnessed, covering, parsed_by_path)
}
