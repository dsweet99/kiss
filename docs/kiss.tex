...
\begin{abstract}
\texttt{kiss} is a lightweight static-analysis tool for feeding coding agents
fast structural feedback during review and refactoring.  It treats metrics as
agent-facing review prompts, not as proof that code is good or bad.  The paper
summarizes which prompts have stronger support from software-engineering
research, which are weaker proxies, and where direct validation on agent-written
code is still needed.  Early measurements suggest that the cheap static
test-name reference check can be much faster than runtime line coverage and may
serve as an effective proxy for finding under-referenced code.
\end{abstract}

\section{Design philosophy}

\texttt{kiss} treats code quality feedback as a set of fast, named, falsifiable
prompts for coding agents rather than as a single quality score.  In this paper,
a metric is a measured quantity, a gate is a thresholded check, and a prompt or
signal is the review interpretation given to the agent.  This follows
Fowler's account of
...
\cite{fowler-code-smell}.  The tool is therefore best understood as an
agent-facing guardrail for review and refactoring, not as an oracle.

Local edits can degrade repository structure, so \texttt{kiss} gives agents
compact feedback before review and refactoring; this remains a design
hypothesis, not a validated law.  That repository-level feedback hypothesis is
...
cost and, in some settings, reduce success, so agent-facing feedback should be
justified, targeted, and minimal
\cite{gloaguen-agents}.

The second non-rejected hypothesis is that internal design quality lowers the
cost of future change.  Fowler argues that developers spend most of their time
modifying existing code, and poor internal quality increases both understanding
time and mistake risk
...
Give agents compact, task-relevant feedback.  Repository-scale tasks need
context, but unfiltered context can add cost or hurt
...
\rowcolor{gray!18}
Static test-name references prove quality & Rejected as a quality target \\
\multicolumn{2}{p{0.86\linewidth}}{
...
\rowcolor{gray!18}
Fast, prioritized agent feedback matters & Non-rejected, partly extrapolated \\
\multicolumn{2}{p{0.86\linewidth}}{
Fast feedback to the agent is consistent with small-step refactoring, and CI
evidence shows that long build durations are a practical problem
\cite{fowler-refactoring,ghaleb}.
...
The confidence column means confidence that the metric is useful as an
agent-facing review or refactoring prompt.  It does not mean that violating the
metric proves the code is bad.  Code smells require contextual judgment
\cite{fowler-code-smell}, and static-analysis warnings are more useful when they
are prioritized for actionability \cite{vassallo,motwani}.

\begin{itemize}
\item \textbf{High confidence} The metric is a strong agent-facing review
prompt. There is direct support from software-engineering research or well-established
...
agent-facing review prompts grounded in construction guidance and broader complexity evidence
...
cycles, and long chains as prompts for agent review.  Information
...
\multicolumn{2}{p{0.86\linewidth}}{Flags definitions whose
names do not appear in tests.  This is not runtime
coverage and should not be read as evidence that a definition is tested.  It is a cheap
...
\subsection{Static test-name reference measurements}

\texttt{inv\_test\_coverage} is the inverted static test-name reference metric behind
...
\emph{median static test-name reference coverage}: the estimated percentage of
...
\begin{longtable}{p{0.12\linewidth}r r r r r r p{0.14\linewidth}r p{0.14\linewidth}}
\toprule
Project & N & p50 & p90 & p95 & p99 & max & Runtime coverage & \texttt{kiss} time &
Coverage time \\
\midrule
\endhead
\texttt{kiss} & 302 & 0 & 0 & 9 & 20 & 75 & 92.60\% &
0.3s & 1.5s \texttt{llvm-cov} \\
\texttt{ripgrep} & 100 & 50 & 100 & 100 & 100 & 100 & 60.47\% &
0.4s & 15s \texttt{llvm-cov} \\
\texttt{rich} & 213 & 0 & 100 & 100 & 100 & 100 & 91\% &
0.2s & 11s \texttt{slipcover} \\
\texttt{ruff} & 2917 & 0 & 0 & 100 & 100 & 100 & 89.48\% &
3.5s & 24m \texttt{llvm-cov} \\
\texttt{ruff} & 1828 & 100 & 100 & 100 & 100 & 100 & 62\% &
-- & 3s \texttt{slipcover} \\
\bottomrule
\end{longtable}
\endgroup
...
execution.  In this small sample, median static test-name reference coverage is often
...
fully agent-written and has been maintained with \texttt{kiss} as a linter.  Its
